...
if (!$done && $_SERVER['REQUEST_METHOD'] === 'POST') {
  $f = $_POST;
  try {
    if (empty($f['admin_user']) || empty($f['admin_pass']) || strlen($f['admin_pass']) < 6) {
      throw new Exception('نام کاربری مدیر و رمز عبور (حداقل ۶ کاراکتر) الزامی است.');
    }
...
<title>نصب سامانهٔ مدیریت سرویس مدارس</title>
<style>
...
<h1>نصب سامانهٔ مدیریت شرکت‌های سرویس مدارس مشهد</h1>
